...
class Tetris extends Component {
  ...
  calculateScores = numLines => {
    ...
    if (numLines) {
      ...
      if (leveledUp) {
        this.dropInterval = this.dropInterval > 100 ? this.dropInterval - 70 : this.dropInterval;
      }
      this.setState({
    ...
    grid.forEach((row, y) => {
      row.forEach((value, x) => {
        if (value !== 0) {
          ...
          this.playContext.fillRect(x * this.scale, y * this.scale, this.scale, this.scale);
        }
      });
    });
  };
  ...
  newGame = () => {
    ...
    this.currentTetromino = new Tetromino(this.currentShape, this.playContext, true, this.scale);
